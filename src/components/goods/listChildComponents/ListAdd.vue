<template>
  <div>
    ...
    <breadcrumb-nav>
      ...
      <template v-slot:secondMenu>添加礼品</template>
    </breadcrumb-nav>

    <!-- 卡片视图 -->
    <el-card>
      <el-alert title="添加礼品信息" type="info" show-icon center :closable="false"></el-alert>

      <!-- 步骤条 -->
      <el-steps :space="200" :active="activeIndex * 1" finish-status="success" align-center>
        <el-step title="基本信息"></el-step>
        <el-step title="礼品参数"></el-step>
        <el-step title="礼品属性"></el-step>
        <el-step title="礼品图片"></el-step>
        <el-step title="礼品内容"></el-step>
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px" label-position="top">
        ...
        <el-tabs v-model="activeIndex" tab-position="left" :before-leave="beforeTabLeave" @tab-click="tabClicked">
          ...
          <el-tab-pane label="礼品参数" name="1">
            <el-form-item v-for="item in manyTableData" :key="item.attr_id" :label="item.attr_name">
              <el-checkbox-group v-model="item.attr_vals">
                <el-checkbox border v-for="(tag,index) in item.attr_vals" v-bind:key="index" :label="tag"></el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </el-tab-pane>
          <el-tab-pane label="礼品属性" name="2">
            <el-form-item v-for="item in onlyTableData" v-bind:key="item.attr_id" :label="item.attr_name">
              <el-input :value="item.attr_vals"></el-input>
            </el-form-item>
          </el-tab-pane>
          <el-tab-pane label="礼品图片" name="3">
            <!-- action表示图片上传的地址 -->
            <el-upload
                    :action="uploadUrl"
          ...
          <el-tab-pane label="礼品内容" name="4">
            <!-- 富文本编辑器组件 -->
            <quill-editor v-model="addForm.goods_introduce" class="editor"></quill-editor>
            <!-- 添加礼品的按钮 -->
            <el-button type="primary" class="addBtn" @click="addGoods">添加礼品</el-button>
          </el-tab-pane>
        </el-tabs>
      </el-form>
    ...
    data() {
      return {
        ...
        addFormRules: {
          ...
          goods_price: [
            {required: true, message: '请输入礼品价格', trigger: 'blur'}
          ],
          goods_number: [
          ...
          goods_cat: [
            {required: true, message: '请选择礼品分类', trigger: 'blur'}
          ]
        },
        categoryList: [],
        categoryProps: {
    ...
    methods: {
      ...
      beforeTabLeave(activeName, oldActiveName) {
        if (oldActiveName === '0' && this.addForm.goods_cat.length !== 3) {
          this.alertMessage('请先选择礼品分类', 'error');
          return false;
        }
      },
      ...
      tabClicked() {
        ...
        if (this.activeIndex === '1') {
          getCategoryParamsRequest(this.cateId, 'many').then(res => {
            let result = res.data;
            if (result.meta.status !== 200) {
              return this.alertMessage('获取礼品参数失败', 'error');
            }
